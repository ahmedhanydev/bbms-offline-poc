    ...
    try {
      ...
      const recentVisits = visits
        ...
        .map((v) => ({
          type: "visit",
          name: v.donor
            ? `${v.donor.firstName} ${v.donor.lastName}`
            : "Unknown",
    ...
    <div className="min-h-screen bg-slate-900 text-white py-8">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          >
            <div className="text-3xl font-bold text-white">
              {isLoading ? "-" : stats.visits.total}
            </div>
            <div className="text-slate-400 text-sm mt-1">Total Visits</div>
            <div className="flex items-center gap-2 mt-2 text-xs">
              <span className="text-green-400">
                {stats.visits.synced} synced
              </span>
              <span className="text-slate-600">|</span>
              <span className="text-yellow-400">
                {stats.visits.pending} pending
              </span>
            </div>
          </Link>
          ...
          >
            <div
              className={`text-3xl font-bold ${
                stats.queue.total > 0 ? "text-yellow-400" : "text-green-400"
              }`}
            >
            ...
            <div className="mt-2 text-xs text-slate-500">
              {stats.lastSync
                ? `Last: ${formatTime(stats.lastSync)}`
                : "Never synced"}
            </div>
          </Link>
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href="/sync"
              className="px-6 py-3 bg-slate-700 hover:bg-slate-600 rounded-lg font-medium transition-colors flex items-center gap-2"
            >
              <span>↻</span>
              <span>Sync Now</span>
            </Link>
          ...
          {isLoading ? (
            ...
            <div className="bg-slate-800 p-8 rounded-lg border border-slate-700 text-center">
              ...
              <p className="text-slate-500 text-sm mt-2">
                Add a donor or record a visit to get started
              </p>
            ...
            <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
              {recentActivity.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50 last:border-b-0"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-slate-400 capitalize">
                        {item.type} added {formatTime(item.date)}
                      </div>
                    </div>
                  </div>
                  <SyncStatusBadge
        ...
        <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
          ...
          <p className="text-slate-400 text-sm">
            This prototype demonstrates offline-first functionality. All data is
            stored locally in IndexedDB and synchronized with the server when
            online. You can continue working offline and sync later.
          </p>

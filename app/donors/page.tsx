...
export default function DonorsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "pending" | "synced" | "failed">(
    "all"
  );
  ...
  const filteredDonors =
    filter === "all"
      ...
      : donors.filter((d) => {
          if (filter === "pending")
            return d.syncStatus === "pending" || d.syncStatus === "local_only";
  ...
  const stats = {
    ...
    pending: donors.filter(
      (d) => d.syncStatus === "pending" || d.syncStatus === "local_only"
    ).length,
